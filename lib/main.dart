...
import 'package:dioascicf/repositories/imc_repository.dart';
        ...
        builder: (BuildContext bc) {
          return AlertDialog(
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  if (controlaPeso.text != '' && controlaAltura.text != '') {
                    await imcRepository.adicionar(IMC(
                      double.parse(controlaPeso.text),
                      double.parse(controlaAltura.text),
                    ));
                    setState(() {});
                    Navigator.pop(context);
                  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext bc, int index) {
                  var imcAtual = _imcs[index];
                  return ListTile(
                    title: Wrap(
                      children: [
                        Text('IMC: ${imcAtual.calculaIMC().toStringAsFixed(2)}'),
                        Text(' - ${imcAtual.classificaIMC()}'),
                      ],
                    ),
                    subtitle: Wrap(
                      children: [
                        Text('Peso: ${imcAtual.peso.toStringAsFixed(2)}'),
                        Text('\tAltura: ${imcAtual.altura.toStringAsFixed(2)}'),
                      ],
                    ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: _adicionaIMC,
        tooltip: 'Adicionar dados',
